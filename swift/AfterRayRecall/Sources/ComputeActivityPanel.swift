...
public struct ComputeActivityPanel<Model: ComputeActivityPresenting>: View {
    ...
    public init(model: Model, onClose: (() -> Void)? = nil) {
        self.model = model
    ...
    public var body: some View {
        ...
        .frame(width: 380)
        .frame(maxHeight: 560)
        .background(RecallPalette.panelDim)
                ...
                HStack(spacing: 6) {
                    Text(gate.workload.title)
                        ...
                        .foregroundStyle(gate.allowed
                            ...
                            : RecallPalette.textSecondary)
                    // The count the user is deciding against: how much is left,
                    // not how much is in flight.
                    if gate.remaining > 0 {
                        Text("\(gate.remaining) left")
                            .font(.system(size: 9, weight: .semibold))
                            .foregroundStyle(gate.allowed
                                ? RecallPalette.textTertiary
                                : RecallPalette.ray)
    ...
    private var controls: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("Chat and anything you are waiting on always runs, whatever is set here.")
                .font(.caption2)
                .foregroundStyle(RecallPalette.textTertiary.opacity(0.8))
                .fixedSize(horizontal: false, vertical: true)
        }
    }
    ...
    private var pauseButtonTitle: String {
        ...
        return "Hold off for an hour"
    }

    private var modeBinding: Binding<ComputeMode> {
        Binding(
            get: { status.mode },
    ...
    private func sectionTitle(_ text: String) -> some View {
        Text(text.uppercased())
            .font(.system(size: 9, weight: .bold))
            .tracking(0.6)
            .foregroundStyle(RecallPalette.textTertiary.opacity(0.9))
    }
}
