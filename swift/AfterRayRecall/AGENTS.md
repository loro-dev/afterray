...
- `Sources/L10n/` — UI i18n (`AfterRayUILanguage`, typed `AfterRayCopy` for `en`/`zh-Hans`/`zh-Hant`/`ja`/`ko`/`es`/`de`/`fr`). `ui_language=auto` follows `Locale.preferredLanguages`. Views take `@Environment(\.afterRayCopy)` via `.afterRayLocalized()`.
