...
- `Sources/RecallStore.swift:4` `RecallStore` — `@MainActor` timeline/playhead state; `Sources/SummaryHistoryStore.swift` owns the independent newest-first history cursor chain. `applyPlayhead` does not publish an unchanged playhead. `RecallImageRepository` (actor) owns artifact byte caching.
...
- `Sources/RecallView.swift:24` `RecallView` — audio/transcript chrome overlays `recallContent`, never the bottom `VStack` (it lifts `DaySummaryPanel`). Read [history-list-scrolling](../../context/history-list-scrolling.md) for history behavior.
- `ArtifactAudioPlayer.swift` + `RecallAudioRepository.swift` + `AudioTimelineFollow.swift` — audio actor prepares/clears uncached buffers off-main. Auto-follow preserves the source; user travel stops it; the root never observes its clock.
- `Sources/AudioMomentChrome.swift` + `AudioMomentTranscript.swift` — exact-segment caption. Aligned cues use binary-search highlighting; estimated legacy cues never highlight. Snapshot with `make audio-chrome-snapshots`.
...
- Every async load guards completion with a generation or request identity; `SummaryHistoryStore` matches both cursor and request ID before publishing.
