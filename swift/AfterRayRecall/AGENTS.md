...
- `Sources/RecallView.swift:24` `RecallView` — the main recall surface (`RecallPalette` at :2619); `AppIdentity` (:1197) is the top-left capsule and `OpenableTitle` (:1256) the window-title line when the frame was a web page. `TranscriptCaption` overlays the timeline (`recallContent`); never put it in the bottom `VStack` or it lifts `DaySummaryPanel`.
- `Sources/ArtifactAudioPlayer.swift` — generation-guarded recall audio; toggle uses last seek origin, not `currentTime`.
