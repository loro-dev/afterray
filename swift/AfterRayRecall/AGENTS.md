...
- `Sources/L10n/` — UI i18n (`AfterRayUILanguage`, typed `AfterRayCopy` for `en`/`zh-Hans`). `ui_language=auto` (and first launch with no `settings.json`) follows `Locale.preferredLanguages`. Views take `@Environment(\.afterRayCopy)` via `.afterRayLocalized()`.
