...
- `protocolVersion` must stay in lockstep with `PROTOCOL_VERSION: u32 = 7` (`crates/afterray-protocol/src/lib.rs:8`); bump both on any wire change or every request fails with `protocolMismatch`.
