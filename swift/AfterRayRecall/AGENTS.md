...
- `Sources/RecallView.swift:24` `RecallView` — the main recall surface (`RecallPalette` at :2619); `AppIdentity` (:1197) is the top-left capsule and `OpenableTitle` (:1256) the window-title line when the frame was a web page. `TranscriptCaption` overlays the timeline (`recallContent`); never put it in the bottom `VStack` or it lifts `DaySummaryPanel`. History (`DaySummaryPanel`) is a virtualized `LazyVStack` with pinned day headers — not one `NSTextView` document; copy is per slot / day / loaded range.
...
- `Sources/ChatAutoScrollState.swift` + `Sources/ChatScrollObserver.swift` — macOS 14 chat follow machine + AppKit geometry bridge. Streaming pins via `defaultScrollAnchor(.bottom)`; the observer is read-only (live-scroll unfollows). `scrollTo` is discrete: open, Latest, new send, one end-of-stream snap.
...
- `Sources/L10n/` — UI i18n. **Chrome must be complete for every shipped locale** (catalog field + every `AfterRayCopy+*.swift` + the live call site). Rules and the `NSHostingView` / English-default pitfalls: [Sources/L10n/AGENTS.md](Sources/L10n/AGENTS.md).
...
- **i18n must stay complete.** User-facing chrome goes through `AfterRayCopy`. Filling every locale is not enough if a view still hardcodes `Text`/`.help`. Host/`NSHostingView` roots use `AfterRayLocalization.shared` (`.afterRayLocalized()` feeds children, not that view). Model APIs keep English defaults for tests; UI calls the `copy:` overload. Contract: [Sources/L10n/AGENTS.md](Sources/L10n/AGENTS.md).
