...
- Every complete daemon prompt gets a fresh `ChatSession`; only the loaded `ModelContainer` persists. Generation runs at temperature 0 with `enable_thinking: false`. See `docs/decisions/active/architecture/2026-08-20-mlx-prefill-and-request-isolation.md`.
...
- Do not key session reuse by system instructions: unrelated summaries intentionally share instructions. Any future continuation needs an explicit protocol conversation identity.
