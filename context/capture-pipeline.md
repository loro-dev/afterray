...
- Schema: `SCHEMA_VERSION = 26`, additive `migrate_schema_N` steps in `migrate`. Key tables: `moments`, `artifacts`, `audio_segments`, `text_evidence`, `evidence_fts` (FTS5), `embeddings` (vector_json + Rust cosine scan), `gop_segments`/`gop_frames`, `slot_summaries`, `summary_slot_geometry`, `input_events`, `edge_snapshots`, `text_df`, `memories`, `conversations`.
- Retention: an optional K-day horizon strips old screenshots, thumbnails, AX trees, complete GOP/audio segments, and edge snapshots while preserving the `moments` timeline rows. The byte cap remains an emergency sweep; in age mode it also strips evidence rather than rows. Raw input content keeps its separate 48-hour privacy deadline. **Any moment-deleting path must call `flush_card_cache`**; `delete_history` still removes overlapping `slot_summaries`.
...
- T2 LLM summaries: `run_slot_t2` driven by a 5-min sweeper (`spawn_slot_summarizer`), gated fail-closed by `t2_may_run` (AC power, ≥30% battery, ≥120s without user input, load/core ≤0.7) and yielding to in-flight OCR. The managed MLX worker loads on demand and exits after 120s without a request.
- Cold storage: `gop_packer.rs` packs stills past the 2-hour hot window into 30-frame closed AV1 GOPs (`afterray-codec`, rav1e). Archive compact also joins consecutive legacy short GOPs when codec, resolution, and capture continuity match and the result stays within 30 frames; mixed-quality sources merge once at a no-better age-appropriate tier. At 7/14/28 days it uses `afterray-gop-decoder` (VideoToolbox) to rewrite old GOPs at progressively stronger constant quantizers. Thumbnails are built before the source JPEG is dropped. Reads still follow thumbnail → still → GOP-frame.
