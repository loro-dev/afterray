...
- Cold storage: `gop_packer.rs` packs stills past the 2-hour hot window into closed AV1 GOPs (`afterray-codec`, rav1e, encode-only). `fold_pack_runs` keeps **one open run per `(width, height)`** so dual-monitor focus flicker does not produce 1-frame still-picture GOPs; `first_packable_run` skips singles. Thumbnails are built in `encode_run` **before** `drop_unpinned_stills` deletes the JPEG, because nothing in Rust can decode AV1 back. Reads follow the thumbnail → still → GOP-frame fallback chain in `read_moment_thumbnail`.
