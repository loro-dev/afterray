...
- Swift mirror: `swift/AfterRayRecall/Sources/DaemonClient.swift` — `UnixSocketDaemonClient`, a hand-declared `WireRequest` with snake_case CodingKeys, and `protocolVersion = 16` enforced on every response. **Bump Rust and Swift together — there is no negotiation; a mismatch fails every request with `protocolMismatch`.** List reads (`timeline_range`, `timeline_list`, `timeline_since`) are a lean index: no concatenated OCR or transcripts. Those stay on `moment_get` / `evidence_ocr`. The overlay fetches one local day at a time and slides that window as the playhead approaches an edge.
...
- `crates/afterray-models/src/persistent_mlx.rs`: `MLX_WORKER_PROTOCOL_VERSION = 2`. A long-lived child speaking newline-delimited JSON both ways: requests `load`/`generate`/`cancel`; responses `ready`/`delta`/`final`/`cancelled`/`error`, each echoing `request_id`. Each `generate` carries a complete, stateless prompt; the protocol has no cross-request KV/session-reuse field. Load timeout 180s, generate 300s, restart backoff 1s. `verify_model` re-checks the pinned manifest + `.afterray-ready.json` marker before every spawn.
- Swift side: `swift/AfterRayMlxVlmWorker/Sources/WorkerCore.swift` — `MlxWorker` actor, `mlxWorkerProtocolVersion = 2`. Single-flight: one `generate` at a time; the model container stays resident but every generate gets a fresh `ChatSession`; `cancel` is acknowledged only after the MLX task actually stops. Revision pins `qwen35_4BRevision`/`qwen35_9BRevision` must stay equal to `QWEN35_4B_MLX_REVISION`/`QWEN35_9B_MLX_REVISION` in `crates/afterray-models/src/catalog.rs` (verified equal today).
