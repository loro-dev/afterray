...
- Swift mirror: `swift/AfterRayRecall/Sources/DaemonClient.swift` — `UnixSocketDaemonClient`, a hand-declared `WireRequest` with snake_case CodingKeys, and `protocolVersion = 15` enforced on every response. **Bump Rust and Swift together — there is no negotiation; a mismatch fails every request with `protocolMismatch`.**
