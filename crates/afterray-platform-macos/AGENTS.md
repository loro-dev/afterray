...
- The shim process gets a short protocol stop window, then kill + reap. After exit 0, its finite stdout reader drains without a local timeout because the bounded channel may be backpressured by required vault imports; only a forced/error reader gets the short abort window. stdout EOF wakes the consumer, but without a preceding protocol `Stopped` it is `UnexpectedEof` and fails the recording; only a real `Stopped` is graceful. See [shutdown-lifecycle](../../context/shutdown-lifecycle.md).
