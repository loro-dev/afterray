...
- `find_slot_mentions` / `match_slot_mention` — index over stored v2 summaries (entities, threads, titles); same `SearchFilter`. `LIKE` prefilters on the query's longest whitespace-free token because the decision happens in `fold_for_match`'s whitespace-free space — a tighter prefilter drops rows silently. `slot_title_covering` uses the row's own `slot_end_ms`; never recompute 30-vs-10-minute bounds.
