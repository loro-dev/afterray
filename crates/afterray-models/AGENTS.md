...
- `src/remote.rs` — `LlmRouterAdapter` (`:127`) routes LLM jobs to a per-pack MLX adapter or Ollama/OpenAI-compatible HTTP; `ModelInput::Llm.temperature` is a per-job remote sampling override (Ollama `options.temperature`, OpenAI-compatible top-level `temperature`; MLX stays deterministic). `check_origin` (`:484`) allows https or loopback-http only; reqwest clients disable redirects so prompts/API keys can't leak to a redirect target. Loopback origins must also bypass proxies (`without_proxy_for_loopback`, `client_for`): reqwest's env lookup stops at the first set of `NO_PROXY`/`no_proxy` — an empty `NO_PROXY=` counts — so a Mac with both sends even 127.0.0.1 to the proxy, which may drop it. Remote generate: connect 2s, headers 15m (time-to-first-byte; several Ollama stacks withhold 200 until the first token, so T2 prefill counts here), 180s idle between chunks — never a total request timeout (`stream.rs`).
