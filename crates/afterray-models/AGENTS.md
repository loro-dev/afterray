...
- `src/process.rs` — one-shot protocol v2; `ProcessAdapter`: one child per inference, one JSON request/response over stdin/stdout, stderr = logs only; 300s timeout, 16 MiB stdout cap.
...
- `src/catalog.rs` — pack specs; `READY_MARKER` (`:4`); pinned MLX revisions + SHA-256 manifests (`QWEN35_4B/9B_MLX_REVISION`) plus the pinned `Qwen3-ForcedAligner-0.6B` snapshot; `model_directory()` (`:97`, `AFTERRAY_MODEL_DIR` override).
...
- GPU lane: background local-GPU jobs serialize across capabilities via `GpuGate`, taken **before** the capability slot; only interactive chat bypasses. The gate mirrors LLM lease holds, so leased rounds ride it deadlock-free. `uses_local_gpu` defaults `true` — remote adapters override. `AFTERRAY_GPU_LANE=0` disables ([why](../../docs/decisions/active/architecture/2026-08-24-gpu-lane-serialization.md)).
...
- **Every pack is SHA-256-pinned** — what makes mirrors safe: MLX and the forced aligner via `HuggingFacePinnedSnapshot`, asr via `SnapshotPin` on `HuggingFaceSnapshot`, embedding via `sha256` on `HuggingFaceFile` (plain layout, no ready marker — installed packs stay valid). To bump: LFS hashes from the HF tree API (`lfs.oid`); small non-LFS files are fetched at that commit and hashed by hand. An `AFTERRAY_*` repo override drops the pin — custom content is knowingly unverified.
