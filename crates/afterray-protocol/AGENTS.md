...
- CLI vs app: `cli_access.rs` classifies each `Request` as Query / Evidence / Privileged. Unprivileged socket peers (`afterray` CLI, agents) get Query always, Evidence only while `cli_evidence_until_ms` is in the future, and never Privileged. The AfterRay app is recognized by executable path (`afterray-platform-macos::peer_is_afterray_app`).
