...
Wire contract for `afterrayd`, the `afterray` CLI, and the SwiftUI app: the `Request`/`Response` envelope, shared payloads, `PROTOCOL_VERSION`, and socket-path rules. Server, chat, and model logic live elsewhere.

## Wire contract

- `Request` enum (`src/lib.rs:22`): `#[serde(tag = "type", rename_all = "snake_case")]`, ~50 variants (`ping`, `status`, `timeline_range`, `search`, `chat_stream`, `shutdown`, …).
- `Response` envelope (`src/lib.rs:275`): `{protocol_version, ok, data?, error?}`; `Response::success`/`failure` always stamp `PROTOCOL_VERSION` (`src/lib.rs:34`).
...
- `PROTOCOL_VERSION` (`src/lib.rs`, currently 21) and `UnixSocketDaemonClient.protocolVersion` (`swift/AfterRayRecall/Sources/DaemonClient.swift`) bump together — Swift rejects mismatches. `gop_quality_preview` uses artifact framing with measured preview metadata in the header.
...
- Compute dashboard types (`ComputeMode`, `ComputeWorkload`, `ComputeLane`, `ComputeGateCode`, `ComputeStatusReport`) sit next to `LlmProvider`. `ComputeLane` exists because macOS publishes no per-process GPU accounting — name the lane, never invent a percentage. (Machine-*wide* utilization is public — the daemon probes it.)
