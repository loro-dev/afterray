...
- `PROTOCOL_VERSION` (`src/lib.rs:34`, currently 18) and `UnixSocketDaemonClient.protocolVersion` (`swift/AfterRayRecall/Sources/DaemonClient.swift`) bump together — Swift rejects mismatches. `Moment.transcript_cues` is detail-only; list/range reads leave it empty.
