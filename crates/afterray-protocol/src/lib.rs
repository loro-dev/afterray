...
/// unprivileged socket clients as a gated query surface. 14 makes the summary
/// slot length a setting (`summary_slot_minutes`). 15 adds the v3 summary
/// body (`details`) to the day summary and the export — additive on the wire,
/// but a client that does not read it renders a v3 card as a title with
/// nothing under it, and a silent empty panel is exactly the failure the
