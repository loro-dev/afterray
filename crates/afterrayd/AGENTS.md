...
- T2: `run_slot_t2` (`T2_MAX_ROUNDS = 8`) + 5-min sweeper gated by `t2_may_run` (AC, ≥30% battery, ≥30s idle, load/core ≤0.7). Writes **card v3** (`put_t2_summary_v3`), grounds citations in code, and sizes its prompt with `t2_prompt_budget_chars(resolve_context_budget())` — never a constant, and the same probe chat uses. `SlotT2Tools` is two tools, one of them audio-gated ([agent-tools](../../context/agent-tools.md#the-t2-slot-surface)).
