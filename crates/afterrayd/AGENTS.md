...
- Capture flow: interval scheduler (default 10s, `AFTERRAY_CAPTURE_INTERVAL_SECONDS`) → `capture_screen` (main.rs:963) → `consume_capture_events` (main.rs:1553) → `import_artifact` (main.rs:1616; screen→`insert_moment`+OCR, audio→`insert_audio_segment`+ASR, AX→exclusion check + `attach_accessibility_snapshot`) → results written back via `insert_text_evidence`, then `submit_embedding` (main.rs:1824).
- `main.rs:1752` — bundle/URL exclusions delete the already-stored moment (`delete_moment_and_artifacts`), because only the AX snapshot carries the URL. Keep that delete-after-capture ordering.
