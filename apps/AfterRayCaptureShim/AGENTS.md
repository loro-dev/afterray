...
- `main.swift:99` `Event` — JSON-line event protocol emitted on stdout (`ready`, `artifact`, `warning`, `failed`, `input_events`, `stopped`)
- `InputEventMonitor` — listen-only tap + coalescing worker (see Invariants)
