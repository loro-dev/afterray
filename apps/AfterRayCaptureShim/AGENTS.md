...
- **Audio exclusions are enforced here, screen exclusions in the daemon.** A moment can be deleted once the snapshot names the app; a finished five-minute `m4a` cannot be sliced, so `ExcludedAudioGate` (`main.swift:894`) drops sample buffers and cuts the open segment while an excluded app is frontmost. The frontmost app is *polled* (100 ms) because the main thread blocks in `readLine` and never services a run loop, so `NSWorkspace` notifications would not arrive.
