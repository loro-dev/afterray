...
- R3 edge snapshots (`captureEdgeSnapshot`, :1801): a frontmost-bundle change or a click arms a candidate, paced by the pure `EdgeSnapshotPacing` (settle 500ms re-armed by any input, ≥5s apart, ≤6/min; a refused candidate is dropped, not queued). Walks the trigger's AXWindow with the same bounded encoder, emits `accessibility_edge`, and **never a screenshot** — an event-driven frame would outlive the 48h events behind it. Excluded apps, AfterRay, and all known browsers are skipped (the private-browsing gate needs an async probe a 1s tick cannot afford). Why: [acts-join](../../context/acts-join.md).
