...
- **跑循环的那个 crate 没有联网的手段。** `afterray-harness` 只有三条依赖：
  `serde`、`serde_json`，以及关掉默认特性、只开 `macros`、`sync`、`time` 的
  `tokio`。没有 HTTP 客户端，没有 socket，也没有运行时。要联网就得先加一条依赖，
  而那会出现在 diff 里。
...
- **工具也不许碰文件系统、开进程、连 socket。** 这一条 Rust 的类型系统表达不了
  ...
  `std::fs`、`std::process` 或 HTTP 客户端，构建就会失败。要绕过它，得去改那份
  清单，改在明面上。
