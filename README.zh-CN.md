...
这些不是承诺，而是有东西挡着，每一条都可以自己核对：

- **agent 循环里没有编译进任何 HTTP 客户端。** 它所在的 `afterray-harness` 只有
  三条依赖：`serde`、`serde_json`，以及关掉默认特性、只开 `macros`、`sync`、
  `time` 的 `tokio`。要讲 HTTP 就得先加一条依赖，而那会出现在 diff 里。但这一条
  本身挡不住裸 socket——`std::net` 不需要任何依赖——那归下面一条管。
- **工具写不了保险库。** 它们拿到的是只读句柄，类型上根本没有 `append`、
  `insert`、`delete`、`clear` 这些方法——写入是编译错误，而不是"碰巧没人这么
  写"。要放宽这个句柄，改动是显式的，而且有测试盯着。
- **工具的参数里没有路径、URL 和命令**，只有时间戳、moment id、查询字符串和条数
  上限。没有可供穿越的东西。
- **工具也不许碰文件系统、开进程、连 socket。** 这一条 Rust 的类型系统表达不了
  ——`std::fs` 在任何 crate 里都是可见的——所以它是一道检查：工具代码里只要出现
  `std::fs`、`std::process`、`std::net` 或 HTTP 客户端，构建就会失败。这是一道
  评审用的 lint，不是沙箱：换个别名或者把代码挪个地方就能绕开。它的作用是让"工具
  拿到了这些能力"这件事没法悄悄发生。
- **屏幕文本在进模型之前会被围栏包起来**，系统提示词也写明围栏里是观察到的数据，
  不是指令。注入正是只读工具面之所以重要的原因：它决定了一次成功的注入最多能做到
  什么。

唯独有一条边界是上面这些都挪不动的：**你把它指向哪个模型**。本地模型意味着提示词
和检索到的证据留在这台 Mac 上；远程端点会收到它们——因为把它们发过去，正是"跑一
个远程模型"这件事本身的含义。完整的图景，包括我们选择接受的那些风险，见
[docs/harness-threat-model.md](docs/harness-threat-model.md)。
