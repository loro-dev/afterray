...
const en = {
  meta: {
    title: 'AfterRay — Not a summary. The moment itself.',
    htmlLang: 'en',
  },
  nav: {
    features: 'Recall',
  ...
  hero: {
    ...
    titleA: ['Not a summary.'] as Part[],
    titleB: ['The ', { em: 'moment itself.' }] as Part[],
    sub: 'AfterRay keeps the actual pixels and audio from your week on this Mac. Replay any second, find it from half a phrase, or just ask — every answer points back to a frame you can watch.',
  ...
  searchAsk: {
    ...
    mock: {
      ...
      queries: [
        {
          ...
          results: [
            ...
            },
            {
              src: 'Transcript',
              time: 'Wed 11:02 AM',
              text: '"favorites never expire — everything else stays bounded" — Zoom',
            },
          ],
        },
        {
      ...
      presets: [
        {
          q: 'Where did we land on retention?',
          a: 'Non-favorites are bounded by a retention ceiling; anything you star is exempt and never expires.',
  ...
  agents: {
    ...
    body: 'The afterray CLI answers in JSON: search, moments, OCR evidence, activity spans, memories, and ask. Install the skill once and Claude Code, Codex, or anything else that reads Agent Skills can query the same surface — read-only, with the vault key never leaving the daemon.',
    points: [
      'afterray search / moment / evidence — raw evidence, as JSON',
...
const zh: Copy = {
  ...
  hero: {
    ...
    titleA: ['不是摘要，'],
    titleB: ['是当时', { em: '那一刻。' }],
    sub: 'AfterRay 把你这一周真实的画面和声音留在这台 Mac 上。回放任意一秒，用记得的只言片语找回来，或者干脆问一句——每个回答都指回一帧你能亲眼看到的画面。',
  ...
  searchAsk: {
    ...
    mock: {
      ...
      presets: [
        {
          q: 'retention 最后怎么定的？',
          a: '非收藏的时刻按保留上限自动清理；只要标了收藏就豁免，永不过期。',
  ...
  agents: {
    titleA: ['一条命令，'],
    titleB: ['任意 ', { em: 'Agent。' }],
    body: 'afterray CLI 用 JSON 回答：检索、时刻详情、OCR 证据、活动片段、记忆与问答。装一次 skill，Claude Code、Codex，以及任何能读 Agent Skills 的工具就能查同一套接口——只读，Vault 密钥永不离开守护进程。',
    points: [
