...
- `event-capture-v2-plan.md` — approved (2026-08-18, in progress): event-driven capture, AX text/diff encoding, screenshot throttling, element-level citations, T2 card v3; records the CAP-005 retirement under the local trust model. WS1–WS3's shim half, WS4 (storage + retention unification + the screenshot throttle), WS5 (card v3, prompt v3, the derived budget) and WS6 have landed; WS7 (the corpus regression) has not. See the "WS2/3 实现偏差", "WS4 实现偏差" and "WS5 实现偏差" sections there, and [context/event-capture-v2.md](../context/event-capture-v2.md) for what the code actually does.
