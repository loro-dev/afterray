...
**`々`、`〆`、`〇` 必须算进连写段**，虽然它们在标点区。它们代替词中的一个表意字（`人々`、`時々`、`二〇二五`），而 Rust 认为它们是字母，所以漏掉它们不只是把段切开，还会把两半当成拉丁词丢进 `AND` 分支——`時々` 于是能命中任何同时含有 時 和 々 的证据行，不管隔多远。

**只有空白能分词。** 被标点连起来的词是**一个 phrase**，不是两个 `AND` 条件：`retry-count` 意思是"这两个词挨着，按这个顺序"，不是"某处有个 retry，另一处有个 count"。FTS5 索引时会把连字符丢掉，phrase 是这个相邻关系唯一的存活方式。CJK 段不参与拼 phrase——它自己的栅栏单字就横在中间，phrase 本来就跨不过去。

**查询整体加引号转义。** 用户打的 `AND`、落单的 `"` 一律当字面量匹配，而不是当语法解析，也就不会再有被吞掉的语法错误。

这三条其实是同一个承诺：**命中的行里必须真的含有这个查询，且是连着的。** 否则跳过去还是没有可高亮的东西——那正是这一整节要解决的问题。

### Schema 18

`evidence_fts` 里 17 之前存的是原文，不重建就永远搜不到。17 本身也要再折一次：`々` 之类进入连写段之后，段的切法变了，17 存下的 `人 々` 已经不是查询会去问的形式了。`migrate_schema_18` 在打开旧库时一次性重折叠整张索引——这是目前唯一一个不能每次开库都重跑的迁移步骤，所以 `migrate` 会在盖版本号**之前**读一次原版本号。
